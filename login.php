...
<head>
    ...
    <script>tailwind.config = { theme: { extend: { colors: { primary: '#3b82f6', primary_dark: '#2563eb' } } } }</script>
    <style>
...
<body class="font-sans min-h-screen flex flex-col items-center justify-center relative overflow-hidden p-4">
    ...
    <main class="w-full max-w-md relative z-10">
        ...
        <div class="bg-white rounded-3xl shadow-xl shadow-slate-200/50 overflow-hidden">
            ...
            <div class="flex border-b border-slate-100 mx-8">
                <a href="/login" class="flex-1 pb-3 text-center text-sm font-bold text-primary border-b-2 border-primary">Đăng nhập</a>
                <a href="/register" class="flex-1 pb-3 text-center text-sm font-semibold text-slate-400 hover:text-slate-600 transition-colors">Đăng ký</a>
            </div>
                ...
                <form id="login-form" class="space-y-5" onsubmit="handleLogin(event)">
                    <div class="space-y-1.5">
                        ...
                        <div class="input-group relative flex items-center bg-slate-50 border-2 border-slate-100 rounded-2xl transition-all duration-300">
                            <div class="pl-4 pr-3 py-3.5"><i data-lucide="user" class="w-5 h-5 text-slate-400 input-icon transition-colors"></i></div>
                            <input type="text" id="username" required class="w-full py-3.5 pr-4 bg-transparent border-none focus:outline-none text-slate-800 font-medium" placeholder="Nhập username">
                        </div>
                    </div>
                    ...
                    <div class="space-y-1.5">
                        ...
                        <div class="input-group relative flex items-center bg-slate-50 border-2 border-slate-100 rounded-2xl transition-all duration-300">
                            <div class="pl-4 pr-3 py-3.5"><i data-lucide="lock" class="w-5 h-5 text-slate-400 input-icon transition-colors"></i></div>
                            <input type="password" id="password" required class="w-full py-3.5 pr-12 bg-transparent border-none focus:outline-none text-slate-800 tracking-wide" placeholder="••••••••">
                            <button type="button" id="toggle-password" class="absolute right-0 inset-y-0 px-4 flex items-center text-slate-400 focus:outline-none"><i data-lucide="eye" id="eye-icon" class="w-5 h-5"></i></button>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center justify-between px-1">
                        <label class="flex items-center gap-2 cursor-pointer select-none">
                            <input type="checkbox" id="remember-me" class="w-4 h-4 rounded border-slate-300 text-primary focus:ring-primary cursor-pointer">
                            <span class="text-sm text-slate-600 font-medium">Ghi nhớ tài khoản</span>
                        </label>
                    </div>
                    ...
                    <button type="submit" id="submit-btn" class="w-full mt-2 bg-gradient-to-r from-primary to-blue-600 text-white font-bold py-4 rounded-2xl shadow-lg active:scale-[0.98] transition-transform flex justify-center items-center relative">
                        <span id="btn-text">Đăng nhập ngay</span>
    ...
    <script>
        ...
        document.getElementById('toggle-password').addEventListener('click', function () {
            const input = document.getElementById('password'); const icon = document.getElementById('eye-icon');
        ...
        async function handleLogin(e) {
            ...
            try {
                ...
                if (result.success) {
                    ...
                    localStorage.setItem('token', result.token || (result.data && result.data.token));
                    localStorage.setItem('username', result.username || (result.data && result.data.username));
                    localStorage.setItem('role', result.role || (result.data && result.data.role));
                    setTimeout(() => {
                        const role = result.role || (result.data && result.data.role);
                        if (role === 'admin') {
                    ...
                    await alert(result.message || 'Đăng nhập không thành công.');
