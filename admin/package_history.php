...
<head>
    ...
    <title>Lịch sử mua gói - TOOLTX2026 Admin</title>
...
<div class="lg:ml-60 min-h-screen flex flex-col">
    <header class="h-16 bg-white border-b border-slate-200 flex items-center px-4 sm:px-6 sticky top-0 z-20 gap-4">
        ...
        <div class="min-w-0">
            <h2 class="font-bold text-slate-800 text-base leading-tight">Lịch sử mua gói VIP</h2>
            <p class="text-xs text-slate-400 hidden sm:block">Xem lại các giao dịch mua gói VIP của thành viên</p>
        </div>
    </header>
    <main class="flex-1 p-4 sm:p-6">
        <div class="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-5 border-b border-slate-100">
                <div>
                    <h3 class="font-bold text-slate-800">Lịch sử giao dịch gói VIP</h3>
                    <p class="text-xs text-slate-400 mt-0.5">Chỉ hiển thị giao dịch loại chi (out) — mua gói</p>
                </div>
                <div class="relative">
                    <i data-lucide="search" class="w-4 h-4 text-slate-400 absolute left-3 top-1/2 -translate-y-1/2 pointer-events-none"></i>
                    <input type="text" id="search-hist" oninput="filterHistory()" placeholder="Tìm username, gói..." class="pl-9 pr-4 py-2 text-sm bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-1 focus:ring-primary w-56">
                </div>
            </div>
                ...
                <table class="w-full text-left min-w-[680px]">
                    <thead class="bg-slate-50 border-b border-slate-100">
                        <tr class="text-slate-400 text-[11px] font-bold uppercase tracking-wider">
                            ...
                            <th class="px-5 py-3">Gói đã mua</th>
                            <th class="px-5 py-3 text-right">Số tiền</th>
                            <th class="px-5 py-3">Thời gian</th>
                            <th class="px-5 py-3">Trạng thái</th>
                        </tr>
                    </thead>
                    <tbody id="history-tbody" class="divide-y divide-slate-50 text-sm">
            ...
            <div class="flex items-center justify-between px-5 py-3 border-t border-slate-100">
                <p id="hist-pg-info" class="text-xs text-slate-400"></p>
                <div id="hist-pg" class="flex items-center gap-1"></div>
            </div>
        </div>
    </main>
</div>
...
function renderHistory(items){
    ...
    if(!items.length){tbody.innerHTML='<tr><td colspan="6" class="py-10 text-center text-slate-400 text-sm">Chưa có lịch sử mua gói.</td></tr>';return;}
    tbody.innerHTML=items.map(tx=>{
        const status=tx.status==='Thành công'?'<span class="px-2 py-1 bg-emerald-50 text-emerald-600 rounded-lg text-[10px] font-bold">Thành công</span>':'<span class="px-2 py-1 bg-red-50 text-red-500 rounded-lg text-[10px] font-bold">'+tx.status+'</span>';
        return `<tr class="hover:bg-slate-50 transition-colors">
            <td class="px-5 py-3.5 font-mono text-xs text-slate-500">${tx.id}</td>
            <td class="px-5 py-3.5 font-semibold text-slate-800">${tx.username}</td>
            <td class="px-5 py-3.5">
                <div class="flex items-center gap-2">
                    <div class="w-7 h-7 bg-orange-50 rounded-lg flex items-center justify-center flex-shrink-0"><i data-lucide="package" class="w-3.5 h-3.5 text-orange-500"></i></div>
                    <span class="text-slate-700 text-sm">${tx.title}</span>
                </div>
            </td>
            <td class="px-5 py-3.5 text-right font-bold text-orange-500">-${formatMoney(tx.amount)}đ</td>
            <td class="px-5 py-3.5 text-xs text-slate-400 whitespace-nowrap">${tx.time}</td>
...
function filterHistory(){
    ...
    filteredHistory=allHistory.filter(t=>t.username.toLowerCase().includes(q)||t.title.toLowerCase().includes(q)||t.id.toLowerCase().includes(q));
    initPagination('hist',{itemsPerPage:20,dataArray:filteredHistory,renderFn:renderHistory,containerId:'history-tbody',paginationId:'hist-pg',infoId:'hist-pg-info'});
}
async function fetchHistory(){
    try{
        ...
        if(d.success){
            allHistory=d.transactions.filter(t=>t.type==='out');
